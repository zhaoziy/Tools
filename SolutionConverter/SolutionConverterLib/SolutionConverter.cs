			...
			using (StreamReader reader = File.OpenText(this.solutionPath))
			{
				...
				foreach (string projectString in projectStrings)
				{
					...
					else
					{
						...
						StreamReader readerCppProj = File.OpenText(projectName);

						Regex ToolsetRegex = new Regex(@"<PlatformToolset>v\d+</PlatformToolset>", RegexOptions.Compiled);
						string Proj = readerCppProj.ReadToEnd();

						readerCppProj.Close();

						Match OldToolset = ToolsetRegex.Match(Proj);
						string oldToolset = OldToolset.Value;
						string newToolset = oldToolset;
						Regex num = new Regex(@"v\d+", RegexOptions.Compiled);

						if (solutionVersion.GetStringValue() == "Format Version 9.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v80");
						}
						else if (solutionVersion.GetStringValue() == "Format Version 10.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v90");
						}
						else if (solutionVersion.GetStringValue() == "Format Version 11.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v100");
						}
						else if (solutionVersion.GetStringValue() == "Format Version 12.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v110");
						}
						else if (solutionVersion.GetStringValue() == "Format Version 12.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v120");
						}
						else if (solutionVersion.GetStringValue() == "Format Version 14.00")
						{
							Match NewToolset = num.Match(newToolset);
							newToolset = newToolset.Replace(NewToolset.Value, "v140");
						}
						Proj = Proj.Replace(oldToolset, newToolset);

						FileStream stream = null;
						TextWriter writer = null;
						try
						{
							stream = File.OpenWrite(projectName);
							writer = new StreamWriter(stream, Encoding.UTF8);
							writer.Write(Proj);
						}
						catch (Exception)
						{
							throw new Exception("工具集修改失败！");
						}
						finally
						{
							this.IsReady = false;
